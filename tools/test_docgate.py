...
def _sample(repo: Path) -> Path:
    ...
    """
    # Centralize the governed fixture path so every baseline assertion names the same file.
...
def test_cli_rerecord_without_reason_exits_nonzero(repo: Path) -> None:
    ...
    """
    docgate.write_baseline(repo)
    # Capture the expected refusal when baseline recording lacks an explicit reason.
...
def test_rerecord_with_reason_updates_only_named_entry(repo: Path) -> None:
    ...
    assert count == 1
    # Reload persisted state to compare the rewritten entry with its untouched peer.
...
def test_load_baseline_accepts_pre_provenance_flat_format(repo: Path) -> None:
    ...
    # Keys are legacy schema fields and file identities; values carry provenance or fingerprints;
...
def test_note_survives_a_rerecord(repo: Path) -> None:
    ...
    # Reload the full document to prove selective entry updates retained global context.
...
def _git(root: Path, *arguments: str) -> subprocess.CompletedProcess[str]:
    ...
    """
    # Capture fixture Git output so setup failures include the exact command diagnostic.
    ...
    # Successful completion is returned for assertions that need commit or blob content.
    return done
...
def git_repo(repo: Path) -> Path:
    ...
    # The initialized repository now has one attributable baseline source commit.
    return repo
...
def test_a_dirty_file_is_never_attributed_to_the_commit(git_repo: Path) -> None:
    ...
    # Inspect the persisted entry whose provenance must fall back to working-tree.
...
def test_every_recorded_ref_replays_to_its_fingerprint() -> None:
    ...
    for name, raw in files.items():
        # Decode each baseline record before deciding whether it claims historical Git bytes.
        entry = docgate.BaselineEntry.from_json(raw, "working-tree")
        if entry.ref == "working-tree":
            # Sentinel entries make no historical blob claim and need no Git verification.
            continue
    ...
    # Each element is a file name lacking both accepted provenance forms; JSON order is immaterial.
    unaccountable = [
...
def test_migrated_baseline_holds_the_original_fingerprints() -> None:
    ...
    """
    # Decode the committed baseline as the migration artifact under test.
    document = json.loads(docgate.BASELINE_PATH.read_text(encoding="utf-8"))
    # Preserve files element values in deterministic source order.
    ...
    # This immutable commit is the provenance inherited by entries not explicitly rerecorded.
    ...
    # Each element is an inherited file name; registry order is retained but not significant.
    ...
    # Keys are rerecorded file names and values are their baseline entries; mapping order is unused.
