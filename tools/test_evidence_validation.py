...
from validate import Layout, Severity, check_evidence
...
def write_matrix(root: Path, *covered: str) -> None:
    ...
    @param root synthetic repository root
    @param covered stable ids the matrix claims to have witnessed
    """
    path = root / "enforce" / "discrimination.py"
    path.parent.mkdir(parents=True, exist_ok=True)
    values = ", ".join(repr(rule_id) for rule_id in covered)
    path.write_text(
        '"""Synthetic discrimination matrix."""\n\n\n'
        "def covered() -> frozenset[str]:\n"
        '    """Return witnessed ids.\n\n    @return ids\n    """\n'
        f"    return frozenset({{{values}}})\n",
        encoding="utf-8",
    )


def evidence_findings(root: Path) -> list[object]:
