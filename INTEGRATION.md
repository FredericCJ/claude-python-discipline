...
gate on a Windows-backed WSL path, it mounts a disposable byte projection held in a
Docker-managed Linux volume, normalizes Python executable intent from shebangs, copies the
JSON report back, and removes the projection. Explicit commands still mount the real
checkout so edits persist. Neither leg governs a parent or sibling repository.
